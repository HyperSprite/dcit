  ...
  if (accConfig.accessCheck(req.user).read !== 1) {
    ...
    if (!req.params.datacenter) {
      ...
      Models.Systemdb.find({}).sort({
        'modifiedOn': 'desc',
      }).exec(function fdcSystemPages00510(err, sys) {
        if (err) {
      ...
      */
      ...
      Models.Optionsdb.find({}, 'optListKey optListArray', function fdcSystemPages00960(err, opt) {
        ...
        Models.Systemdb.find({}, {
        ...
        }, function fdcSystemPages01070(err, sysName) {
          if (err) return next(err);
          ...
          }, function fdcSystemPages01240(err, eq) {
            if (err) return next(err);
            if (!eq) return next();
            // logger.info('rk'+rk);
            var eqUni = [];
      ...
      */
      ...
      Models.Systemdb.find({}, {
        ...
        Models.Systemdb.findOne({
        ...
        }, function fdcSystemPages02100(err, sy) {
          if (err) return next(err);
          if (!sy) return next();
          ...
          Models.Optionsdb.find({}, 'optListKey optListArray', function fdcSystemPages02170(err, opt) {
            if (err) return next(err);
            ...
            Models.Equipment.find({}, {
            ...
            }, function fdcSystemPages02360(err, eq) {
              ...
              Models.Datacenter.findOne({
                ...
                'networks': 1,
                '_id': 0
              }, function fdcSystemPages02470(err, oneDC) {
                ...
                if (editLoad < 4) { // Edit or View
                  ...
                  if (thisEquip.equipMake && thisEquip.equipModel !== false) {
                    ...
                    thisEquipPortsMapped = thisEquip.equipPorts.map(function fdcSystemPages02890(tep) {
                      return {
                  ...
                  context = {
                    ...
                    modifiedOn: strTgs.dateMod(sy.modifiedOn),
                    systemPorts: sy.systemPorts.map(function fdcSystemPages03480(sp) {
                      var isConsole,
                        isEthernet,
                        isInfiniband,
                        isInterconnect,
                  ...
                  context = {
                    ...
                    systemStart: strTgs.dateMod(sy.systemStart),
                    systemEnd: strTgs.dateMod(sy.systemEnd),
                    systemPorts: sy.systemPorts.map(function fdcSystemPages0474(sp) {
                      return {
                        sysPortType: sp.sysPortType,
                        sysPortName: sp.sysPortName,
                        sysPortVlan: sp.sysPortVlan,
    ...
    if (!fEndPoint) {
      ...
      Models.Systemdb.find({
        ...
        context = {
          ...
          sys: sys.map(function ffindEndpoints11380(sy) {
            return {
              systemPorts: sy.systemPorts.map(function ffindEndpoints11400(sysPort) {
...
findOne({
        ...
        sys.save(function(err) {
          ...
          if (thisDoc.systemName !== strTgs.clTrim(bd.systemName)) {
            ...
            }, 'systemName systemPorts.sysPortName systemPorts.sysPortEndPoint', function(err, sys) {
              sys.map(function(sy) {
...
exports.dcsystemDelete = function(req, res) {
  if (accConfig.accessCheck(req.user).delete !== 1) {
    ...
    if (req.body.systemName) {
      ...
      }, function(err, systemNametodelete) {
        if (err) {
          logger.warn(err);
...
exports.dcsystemSubDelete = function(req, res) {
  ...
  if (accConfig.accessCheck(req.user).delete !== 1) {
    ...
    if (req.body.id && req.body.subId) {
      Models.Systemdb.findById(req.body.id, req.body.subDoc, function(err, sys) {
        if (err) {
