...
//this code expects jquery, will not work here
exports.phoneFormat=function (phonenum) {
...
// Used in Options lists
// this cleans up new values remvoes spaces
exports.sTrim = function(current){
    if (typeof current === "undefined"){
    } else {
        current = current.replace(/[^a-z0-9_\-]/gi, ""); 
        console.log("sTrim >"+current);
    }
    return current;
};

// Used in Options lists
// this cleans up new values
exports.uTrim = function(current){
    if (typeof current === "undefined"){
    } else {
        current = current.replace(/[^a-z0-9_\- ]/gi, "");
        current = current.trim();
        console.log("uTrim >"+current);
    }
    return current;
};
// used everywhere
...
exports.uCleanUp = function(old,current){
    ...
    if(old==="" && current==""){
        console.log("uCleanup - old=== & new== ");
    }else if (typeof current ==="undefined"){
        console.log("uCleanup - typeof used");
    }else{
        console.log("uCleanup - current trim");
...
exports.csvCleanup = function(csv){
    console.log("csvCleanup");
        csv = csv.split(",");
    for(i=0;i<csv.length;i++){
        console.log("before >"+csv[i]);
        csv[i] = exports.uTrim(csv[i]);
        console.log("after  >"+csv[i]);
        }
        return csv;
};
...
exports.compUs = function(rUs,dc,cage,rack){
    ...
    var arr = new Array;
    if (isNaN(rUs)){
        console.log("rUs undefiend");
    } else {
    console.log("rUs defined");
        for(i=0;i<rUs;i++){
            //console.log(dc+"_"+cage+"_"+rack+"_"+exports.pad(i));
            arr[i] = dc+"_"+cage+"_"+rack+"_"+exports.pad(i);
            console.log("making rUs :"+ arr[i]);
        }
    console.log("rUs requested:"+rUs);    
    }
    return arr;
};
...
exports.findThisInThat = function(findThis,inThat){
    var data;
    if(!findThis){
    console.log("findThisInThat.findThis is null");
    }else{
    // Get Index of object based on id value
    console.log("findOne  >"+findThis);
    ...
    console.log("inThat[arrayPosition]");
    data = false;
    }else{
...
// cheeting, this could be better, this could break if someone has a 100 U rack
...
exports.ruToLocation = function(ruToLoc){
    if(!ruToLoc){console.log("err");}else{
...
// handlebars will display "checked" if checked, nthing if false
// we'll see if that works for the submt
exports.setCheckBox = function(startVal){
    console.log("setCheckBox sV>"+startVal);
    var newVal = "";
    if(startVal===true){
        newVal = "checked";
        }
    console.log("setCheckBox nV>"+newVal);    
    return newVal;
};
...
// used in Equipment List
// this takes a true or false value and returns a red or green icon in Public/Img/
...
/*
exports.trueFalseIcon = function(test){
    if(test === true){
        test = "<img src='/img/true.png'>";
    }else{
        test = "<img src='/img/false.png'>";
    }
    return test;
};

exports.statusIcon = function(test){
    if(test === "In Service"){
        test = "<img src='/img/true.png'>";
    }else if(test === "In Service with Issues"){
        test = "<img
    }
};
*/
...
// this is used for the Datacenter list
...
exports.findCGParent = function (child,dc){
    console.log("uber child: "+child);
    console.log("uber dc:    "+dc);
...
// takes single and makes it an array  or "toarray"
    if(!dc.length){
       dc = [dc]; 
        ...
        var uber;
        for(i=0;i<dc.length;i++){
    console.log("uber for i: "+dc[i].abbreviation);
            for(j=0;j<dc[i].cages.length;j++){
                if(dc[i].cages[j].id == child){
    console.log("uber for j: "+dc[i].abbreviation);
                    uber ={
...
exports.findOneCGParent = function (child,dc){
    ...
    console.log("uber for j: "+dc.abbreviation);
                    uber ={
...
// this is to set the default select, may move this to a dedicated Handlebars file
