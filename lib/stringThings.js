...
exports.dateMod = function (s) {
    ...
    dateResults = theyear+'-'+themonth+'-'+thetoday;
        logger.info('dateResults '+dateResults);
    return dateResults;
    }
};
*/
...
/*
// working for MAC addresses entry, so it goes in proper
// this cleans up new values removes all spaces, makes them all caps
exports.mTrim = function(current){
    if (typeof current === 'undefined'){
    }else{
    ...
    var mA=[];
        mA = current.match(/([a-fA-F0-9]{2})/g);
        current = mA[0]+':'+mA[1]+':'+mA[2]+':'+mA[3]+':'+mA[4]+':'+mA[5];
        current = current.toUpperCase();
        logger.info('mTrim >'+current);
    }    
    return current;
...
exports.mTrim = function(current){
    ...
    logger.info('mac '+current);
    //var macTest = /\d{2}:\d{2}:\d{2}:\d{2}:\d{2}:\d{2}/;
    var mA=[];
    if (current.length === 12){
        //current = current.replace (/(\(\w\w\)\(\w\w\)\(\w\w\)\(\w\w\)\(\w\w\)\(\w\w\)/)\1:\2:\3:\4:\5:\6/g);
        //current = current.replace(/([a-fA-F0-9]{2})/g,'$&:');
        mA = current.match(/([a-fA-F0-9]{2})/g);
        current = mA[0]+':'+mA[1]+':'+mA[2]+':'+mA[3]+':'+mA[4]+':'+mA[5];
    }else {
        current = current.replace(/[^a-fA-F0-9][:]/gi, '');
