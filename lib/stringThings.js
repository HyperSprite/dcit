...
exports.dateAddTZ = function (theDate, userTZ) {
  ...
  if (moment(theDate).isValid() === false) {
    logger.info('dateAddTZ is === false');
    return null;
  }
  logger.info(momentTZ(theDate, userTZ).format());
  return momentTZ(tempDate, userTZ).format();
};
...
// usage in the handler strTgs.multiTrim(value to trim, number for option, upper/lower case)
...
// takes up to 4 arguments and returns them as a single space sep string
// not used
module.exports.strClng = (str1, str2, str3) => {
  var result;
  result = str1 || '';
  if (str2) {
    result += ' ' + str2;
  ...
  console.log('strClng' + result);
  return result;
};

module.exports._compact = (obj) => {
