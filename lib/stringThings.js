...
exports.multiTrim = function(current,opts){
    if (typeof current === 'undefined'){
    } else {
        ...
        switch(opts){
            ...
            //logger.info('multiTrim 2');
                ...
                current = current.toLowerCase();
            break;
            case 3: // letters, numbers "-" "_" "~" no spaces replaces stTrim
                current = current.replace(/[^a-z0-9.~_\-]/gi, '');
            break;
            case 4: // letters, numbers "-" "_" "~" no spaces replaces stcTrim
                current = current.replace(/[^a-z0-9.,~_\-]/gi, '');
            break;
...
exports.findThisInThatMulti = function(findThis,inThat,multi){
    var data;
    logger.info('fTiT multi>' + multi);
    if(!findThis){
    logger.info('findThisInThat.findThisMulti is null');
    data = false;
    }else{
    // Get Index of object based on id value
    logger.info('findOne  >'+findThis);
    var arrayPosition = inThat.map(function(arrayItem){
    // Match our findThis to inThat Object    
    return arrayItem[multi]; }).indexOf(findThis);
    if(!inThat[arrayPosition]){
    //logger.info('inThat[arrayPosition]');
    data = false;
    }else{
...
// inThat is the Systemdb collection with minimized results 
/* no longer used
exports.findThisInThat3 = function(findThis,inThat){
    var data;
    if(!findThis){
    //logger.info('findThisInThat.findThis is null');
    data = false;
    ...
    var arrayPosition = inThat.map(function(arrayItem){
    // Match our findThis to inThat Object    
    return arrayItem.systemName; }).indexOf(findThis);
    if(!inThat[arrayPosition]){
    //logger.info('inThat[arrayPosition]');
    data = false;
    }else{
...
*/
// This is for the Options from the Opt DB for drop downs
...
exports.findThisInThatNetwork = function(findThis,inThat){
    ...
    return Number(arrayItem.dcNetVlan); }).indexOf(Number(findThis));
    if(!inThat[arrayPosition]){
